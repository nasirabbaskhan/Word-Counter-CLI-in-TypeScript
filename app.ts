...
(async () => {
  await showBanner(
    "Word Counter",
    "Word Counter is used count the words of your Given Sentences",
    "red",
    "blue"
  );
})();
...
async function fun() {
  ...
  while (loop) {
    const answer: Answer = await inquirer.prompt([
      {
        type: "input",
        name: "userSentence",
        message: gradient.rainbow(
          "Kindly enter your sentence to count the words"
        ),
      },
    ]);
    ...
    console.log(
      gradient.rainbow(`the total words of your sentances are: ${totalwords}`)
    );
